...
public class App {
	...
	private static void resektatDokumentieren(Resektat r) {
		...
		C.print("->  Geben Sie die apiko-basale L�nge in Millimetern an:");
		r.setApikoBasal(C.inputDouble(1, 1000, 2));
		C.print("->  Geben Sie die horizontale L�nge in Millimetern an:");
		r.setHorizontal(C.inputDouble(1, 1000, 2));
		C.print("->  Geben Sie die antero-dorsale L�nge in Millimetern an:");
		r.setAnteroDorsal(C.inputDouble(1, 1000, 2));
		
		C.print("==> Prostata-Ma�e erfasst! Beginnen Sie mit dem Zuschnitt.");
		C.print("->  Schneiden Sie den Apex ab und zerteilen Sie ihn in 2 H�lften...");
		C.print("->  Anschlie�end zerschneiden Sie die beiden Teilh�lften in beliebig viele St�cke.");
		C.print("->  Fortfahren (Enter)");
		C.pressEnter();
		
		C.print("->  Beginne mit der Erfassung der Objekttr�ger f�r den Apex...");
		r.setApex(new Apex());
		...
		C.print("->  Anschlie�end zerschneiden Sie die Teilh�ften jeder Scheibe in beliebig viele St�cke.");
		C.print("->  Fortfahren (Enter)");
		C.pressEnter();
		
		for (int i = 0; i < anzahlScheiben; i++) {
			C.print("->  Beginne mit der Erfassung der Objekttr�ger f�r Scheibe " + (i+1) + "...");
		...
		C.print("==> Scheiben wurden erfasst! Zuletzt zerteilen Sie die Basis in 2 H�lften.");
		C.print("->  Anschlie�end zerschneiden Sie die Teilh�lften in beliebig viele St�cke.");
		C.print("->  Fortfahren (Enter)");
		C.pressEnter();
		
		C.print("->  Beginne mit der Erfassung der Objekttr�ger f�r die Basis...");
	...
	private static void objekttraegerErfassen(Scheibe s, boolean istRechteHaelfte) {
		// Information um welche H�lfte es geht (f�r console output)
		String haelfteInfo;
		if(istRechteHaelfte)
			haelfteInfo = "rechte H�lfte";
		else
			haelfteInfo = "linke H�lfte";
		// Wie viel St�cke wurden erzeugt?
		C.print("->  In wie viele St�cke haben Sie die "+haelfteInfo+" zerteilt?");
		int anzahl = C.inputInt(1, 10);
		// Objekttr�ger erfassen
		C.print("->  Geben Sie die Objekttr�ger f�r die einzelnen St�cke an:");
		for (int i = 0; i < anzahl; i++) {
			C.print("Objekttr�ger f�r St�ck "+(i+1)+" ("+haelfteInfo+"):");
		...
		C.print("==> Objekttr�ger wurden f�r die "+haelfteInfo+" erfasst!");
	}
}
